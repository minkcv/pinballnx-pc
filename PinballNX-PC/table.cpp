...
Table::Table(SceneElement* root, b2World& world) : 
    m_layers(),
    m_ramps(),
    m_bumpers(),
    m_spinners(),
    ...
    m_wheels(),
    m_scoreboard(root),
    m_leftFlipper(root, world, 0),
    m_rightFlipper(root, world, 1),
    m_leftFlipper2(root, world, 2),
    m_rightFlipper2(root, world, 3),
    m_plunger(root, world),
    m_announce()
{
    m_b2world = &world;
    m_root = root;
    ...
    Layer underLayer(root, world, 0);
    m_layers.push_back(underLayer);

    Layer launchTubeLayer(root, world, 1);
    m_layers.push_back(launchTubeLayer);

    Layer mainTableLayer(root, world, 2);
    m_layers.push_back(mainTableLayer);

    // Rails
    Layer layer2(root, world, 3);
    m_layers.push_back(layer2);

    // Ramps for main layer
    Ramp ramp1(root, world, 0, 2, 3);
    m_ramps.push_back(ramp1);

    Ramp ramp1up2(root, world, 1, 2, 3);
    m_ramps.push_back(ramp1up2);

    Ramp ramp1Down(root, world, 2, 3, 2);
    m_ramps.push_back(ramp1Down);

    Ramp hole1(root, world, 3, 3, 2);
    m_ramps.push_back(hole1);

    Ramp rightRamp(root, world, 4, 2, 3);
    m_ramps.push_back(rightRamp);

    Ramp rightRampUp2(root, world, 5, 2, 3);
    m_ramps.push_back(rightRampUp2);

    Ramp rightRampDown(root, world, 6, 3, 2);
    m_ramps.push_back(rightRampDown);
    
    Ramp hole2(root, world, 7, 3, 2);
    m_ramps.push_back(hole2);

    Ramp ramp3(root, world, 8, 2, 3);
    m_ramps.push_back(ramp3);

    Ramp ramp3Down(root, world, 9, 3, 2);
    m_ramps.push_back(ramp3Down);

    Ramp ramp3up2(root, world, 10, 2, 3);
    m_ramps.push_back(ramp3up2);

    Ramp backdoorDown(root, world, 11, 1, 2);
    m_ramps.push_back(backdoorDown);

    Ramp tunnelDown(root, world, 12, 2, 0);
    m_ramps.push_back(tunnelDown);

    Ramp newballEntrance2(root, world, 13, 1, 2);
    m_ramps.push_back(newballEntrance2);

    Ramp newballEntrance3(root, world, 14, 1, 2);
    m_ramps.push_back(newballEntrance3);

    Ramp newballEntrance4(root, world, 15, 1, 2);
    m_ramps.push_back(newballEntrance4);

    Ramp underLayerUp(root, world, 16, 0, 2);
    m_ramps.push_back(underLayerUp);

    Ramp topRailEntrance(root, world, 17, 3, 2);
    m_ramps.push_back(topRailEntrance);

    BallLock* ballLock = new BallLock(root, world, 2, 0, 4, true);
    m_ballLocks.push_back(ballLock);

    BallLock* leftLock = new BallLock(root, world, 2, 1, 2);
    m_ballLocks.push_back(leftLock);

    BallLock* middleLock = new BallLock(root, world, 2, 2, 1);
    m_ballLocks.push_back(middleLock);

    BallLock* upperMiddleLock = new BallLock(root, world, 2, 3, 1);
    m_ballLocks.push_back(upperMiddleLock);

    BallLock* underBallLock1 = new BallLock(root, world, 0, 4, 4);
    m_ballLocks.push_back(underBallLock1);

    BallLock* underBallLock2 = new BallLock(root, world, 0, 5, 4);
    m_ballLocks.push_back(underBallLock2);

    BallLock* railLock = new BallLock(root, world, 3, 6, 1);
    m_ballLocks.push_back(railLock);

    BallLock* jackpotLock = new BallLock(root, world, 0, 7, 4);
    m_ballLocks.push_back(jackpotLock);

    OptWall* leftRailWall = new OptWall(root, world, 0, 3);
    m_optWalls.push_back(leftRailWall);

    OptWall* underLayerLock1 = new OptWall(root, world, 1, 2);
    underLayerLock1->enable();
    m_optWalls.push_back(underLayerLock1);

    OptWall* underLayerLock2 = new OptWall(root, world, 2, 0);
    m_optWalls.push_back(underLayerLock2);

    OptWall* leftKickerLock = new OptWall(root, world, 3, 2);
    m_optWalls.push_back(leftKickerLock);

    OptWall* rightKickerLock = new OptWall(root, world, 4, 2);
    m_optWalls.push_back(rightKickerLock);

    OptWall* rightLock = new OptWall(root, world, 5, 2);
    rightLock->enable();
    m_optWalls.push_back(rightLock);

    OptWall* topLeftLock = new OptWall(root, world, 6, 2);
    topLeftLock->enable();
    m_optWalls.push_back(topLeftLock);

    OptWall* leftRailWall2 = new OptWall(root, world, 7, 3);
    leftRailWall2->enable();
    m_optWalls.push_back(leftRailWall2);

    OptWall* rightLockLower = new OptWall(root, world, 8, 2);
    m_optWalls.push_back(rightLockLower);

    Trigger* leftTrigger = new Trigger(root, world, 0, 2, 0, 0, underLayerLock1, leftKickerLock, rightKickerLock);
    m_triggers.push_back(leftTrigger);

    Trigger* topRightTrigger = new Trigger(root, world, 1, 2, 0, 0, nullptr, leftKickerLock, rightKickerLock);
    m_triggers.push_back(topRightTrigger);

    Trigger* topLeftTrigger = new Trigger(root, world, 2, 2, 2, 0, leftRailWall, topLeftLock, leftRailWall2);
    m_triggers.push_back(topLeftTrigger);

    Trigger* lowerRightTrigger = new Trigger(root, world, 3, 2, 2, 15, rightLock, rightLockLower);
    m_triggers.push_back(lowerRightTrigger);

    // Circle bumpers at the top left
    Bumper* bumper1 = new Bumper(root, world, 2, -1, 8.8, 2.2);
    m_bumpers.push_back(bumper1);

    Bumper* bumper2 = new Bumper(root, world, 2, -1, 9.9, 1.9);
    m_bumpers.push_back(bumper2);

    Bumper* bumper3 = new Bumper(root, world, 2, -1, 9.6, 3.0);
    ...
    Bumper* underBumper = new Bumper(root, world, 0, 6, 0, 0, underLayerLock2, 1);
    m_bumpers.push_back(underBumper);

    Bumper* underBumper2 = new Bumper(root, world, 0, 7, 0, 0, nullptr, 1);
    m_bumpers.push_back(underBumper2);

    // Bumpers for return area bumpers
    Bumper* bumperLeftUpper = new Bumper(root, world, 2, 0);
    m_bumpers.push_back(bumperLeftUpper);

    Bumper* bumperLeftLower = new Bumper(root, world, 2, 1);
    m_bumpers.push_back(bumperLeftLower);

    Bumper* bumperRightUpper = new Bumper(root, world, 2, 2);
    m_bumpers.push_back(bumperRightUpper);

    Bumper* bumperRightLower = new Bumper(root, world, 2, 3);
    m_bumpers.push_back(bumperRightLower);

    // Kicker bumpers
    // HACK: the x and y args aren't used if we pass a positive ShapeId so we just pass 0
    Bumper* bumperLeftKicker = new Bumper(root, world, 2, 4, 0, 0, leftKickerLock, 1);
    m_bumpers.push_back(bumperLeftKicker);

    Bumper* bumperRightKicker = new Bumper(root, world, 2, 5, 0, 0, rightKickerLock, 1);
    m_bumpers.push_back(bumperRightKicker);

    Spinner* topLeftSpinner = new Spinner(root, world, 2, 0);
    m_spinners.push_back(topLeftSpinner);

    GTarget* rightTargets = new GTarget(root, world, 0, 2, false, true);
    m_gtargets.push_back(rightTargets);

    GTarget* rightReturnTargets = new GTarget(root, world, 1, 2, true);
    m_gtargets.push_back(rightReturnTargets);

    GTarget* leftReturnTargets = new GTarget(root, world, 2, 2, true);
    m_gtargets.push_back(leftReturnTargets);

    GTarget* middleTargets = new GTarget(root, world, 3, 2, false, true);
    m_gtargets.push_back(middleTargets);

    GTarget* launchTargets = new GTarget(root, world, 4, 2, true);
    m_gtargets.push_back(launchTargets);

    GTarget* multiTargets = new GTarget(root, world, 5, 2, false, false, true);
    m_gtargets.push_back(multiTargets);

    PTarget* rampTargets = new PTarget(root, world, 0, 2);
    m_ptargets.push_back(rampTargets);

    PTarget* holeTargets = new PTarget(root, world, 1, 2);
	...
	rect->setSize(Vector2f(200, 720));
	rect->setFillColor(Color::Black);
	rect->setPosition(Vector2f(-200, 0));
	SceneElement* rectElt = new SceneElement(rect);
	rectElt->setLayer(100);
	root->add(rectElt);
}
...
void Table::update() {
    ...
    for (size_t i = 0; i < m_pinballs.size(); i++) {
        ...
        pinball->update(m_root, m_b2world);
    ...
    if (m_lockBallTimers.size() > 0) {
        for (size_t i = 0; i < m_lockBallTimers.size(); i++) {
            ...
            if (timer < 0) {
                ...
                Pinball* lockBallRelease = new Pinball(m_root, m_b2world, iSpawnPos);
                if (iSpawnPos == 4)
    ...
    if (m_multiCreate > 0) {
        if (m_multiTimer < 0) {
            Pinball* multiBall = new Pinball(m_root, m_b2world, 4);
    ...
    m_leftFlipper.update();
    m_rightFlipper.update();
    m_leftFlipper2.update();
    m_rightFlipper2.update();
    m_plunger.update();
    ...
    for (size_t i = 0; i < m_pinballs.size(); i++) {
        ...
        for (size_t b = 0; b < m_ballLocks.size(); b++) {
            ...
            if (!(pinball->isOut()) && ((fixtureA == ballLock->getFixture() && fixtureB == pinball->getFixture()) ||
                ...
                m_score += 20000;
				if (b == 1) {
					// Open the optwall that blocks this hole
					m_optWalls.at(0)->disable();
					m_optWalls.at(7)->enable();
					m_optWalls.at(6)->disable();
